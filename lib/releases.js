    ...
    pullRequests
      .map((pullRequest) =>
        template(config['change-template'], {
          $TITLE: pullRequest.body
            ? (pullRequest.body.includes('<changelog>') // eslint-disable-line prettier/prettier
              ? pullRequest.body.slice(
                  pullRequest.body.indexOf('<changelog>') + 11,
                  pullRequest.body.indexOf('</changelog>') -
                    pullRequest.body.indexOf('<changelog>')
                )
              : escapeTitle(pullRequest.title)) // eslint-disable-line prettier/prettier
            : escapeTitle(pullRequest.title),
